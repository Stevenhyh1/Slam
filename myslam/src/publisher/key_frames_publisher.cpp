...
KeyFramesPublisher::KeyFramesPublisher(ros::NodeHandle &nh, std::string topic_name, std::string frame_id, size_t buff_size): nh_(nh), frame_id_(frame_id) {
...
void KeyFramesPublisher::Publish(std::deque<KeyFrame> key_frames) {
    ...
    LOG(INFO) << "Size of the path: " << path.poses.size() << std::endl;
