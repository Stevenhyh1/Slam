...
BackEndFlow::BackEndFlow(ros::NodeHandle& nh) {
    ...
    gnss_pose_sub_ptr_ = std::make_shared<OdometrySubscriber>(nh, "/synced_gnss", 10);
    laser_odom_sub_ptr_ = std::make_shared<OdometrySubscriber>(nh, "/laser_odom", 10);
...
bool BackEndFlow::ForceOptimize() {
    ...
    if (back_end_ptr_->HasNewOptimized()) {
        ...
        back_end_ptr_->GetOptimzedKeyFrames(optimized_key_frames);
