...
int main(int argc, char *argv[]) {
    ...
    ros::init(argc, argv, "myslam_node");
    ros::NodeHandle nh;
    
    ROS_INFO("Node Started.");

    std::shared_ptr<CloudSubscriber> cloud_sub_ptr = std::make_shared<CloudSubscriber>(nh, "/kitti/velo/pointcloud", 10);
    std::shared_ptr<IMUSubscriber> imu_sub_ptr = std::make_shared<IMUSubscriber>(nh, "/kitti/oxts/imu", 10);
    std::shared_ptr<GNSSSubscriber> gnss_sub_ptr = std::make_shared<GNSSSubscriber>(nh, "/kitti/oxts/gps/fix", 10);
    ...
    while (ros::ok())
    {
        ...
        if (!transform_received) {
            if (lidar_to_imu_ptr->LookupData(lidar_to_imu)) {
                ...
                ROS_INFO("lidar to imu transfrom matrix is: ");
            ...
            while (!cloud_data_buff.empty() && !imu_data_buff.empty() && !gnss_data_buff.empty()) {
                ...
                // std::cout << "Size of Point Cloud Data: " << cloud_data_buff.size() << std::endl;
                // std::cout << "Size of IMU Data: " << imu_data_buff.size() << std::endl;
                // std::cout << "Size of GNSS Data: " << gnss_data_buff.size() << std::endl;
