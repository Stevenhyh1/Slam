...
int main(int argc, char **argv) {
    ...
    for (int v = 0; v < rows; v++)
        for (int u = 0; u < cols; u++) {
            ...
            double x_distorted = x * (1 + r_2 * k1 + r_2 * r_2 * k2);
            double y_distorted = y * (1 + r_2 * k1 + r_2 * r_2 * k2);
            ...
            // cout << image_undistort.at<uchar>(v, u) << endl;
        }

    cv::imshow("image undistorted", image_undistort);
    cv::imwrite("../undistored.png", image_undistort);
    cv::waitKey();
